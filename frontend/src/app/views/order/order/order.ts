...
export class Order implements OnInit {
  ...
  ngOnInit(): void {
    this.cartService.getCart().subscribe((data: CartType | DefaultResponseType) => {
      ...
      if (!this.cart || (this.cart && this.cart.items.length === 0)) {
        this._snackBar.open('Необходимо добавить товары в корзину!', undefined, { duration: 2500 });
  ...
  createOrder() {
    if (
      ...
      this.orderService.createOrder(paramsObject).subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          if (error.error && error.error.message) {
            ...
            this._snackBar.open('Ошибка заказа', undefined, {
              duration: 2500,
            });
          }
        },
      ...
      this._snackBar.open('Заполните необходимые поля', undefined, {
        duration: 2500,
      });
    }
  }
